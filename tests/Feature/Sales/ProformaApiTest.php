...
class ProformaApiTest extends TestCase
{
    ...
    {
        $admin = $this->user('admin'); $withoutRecipe = Hamaca::create(['nombre' => 'Sin fórmula '.uniqid(), 'categoria_id' => DB::table('categorias')->value('id'), 'tamano_id' => DB::table('tamanos')->value('id'), 'precio' => 800]); Sanctum::actingAs($admin);
        $this->postJson('/api/v1/proformas', $this->payload($withoutRecipe, $admin->id))->assertStatus(422)->assertJsonPath('message', 'La hamaca seleccionada no tiene una fórmula activa.');
        $hamaca = $this->hamacaWithRecipe(); $products = $this->getJson('/api/v1/proformas/productos')->assertOk();
        $this->assertTrue(collect($products->json('data'))->contains(fn ($row) => $row['id'] === $hamaca->id && !array_key_exists('variante', $row)));
        $this->assertArrayNotHasKey('hamaca_variante_id', $this->payload($hamaca, $admin->id)['detalles'][0]);
    }
