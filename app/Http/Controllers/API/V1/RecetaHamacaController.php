...
class RecetaHamacaController extends Controller
{
    ...
    public function index(Hamaca $hamaca)
    {
        return RecetaHamacaResource::collection(
            $hamaca->recetas()->with(['detallesMateriales.material', 'detallesManoObra.proceso'])->latest('version')->get()
        );
    }

    public function active(Hamaca $hamaca)
    {
        $recipe = $hamaca->recetaActiva()->with(['detallesMateriales.material', 'detallesManoObra.proceso'])->first();
    ...
    {
        return (new RecetaHamacaResource($this->recetaService->crearBorrador($hamaca, $request->user())))
            ->response()
            ->setStatusCode(201);
    }
