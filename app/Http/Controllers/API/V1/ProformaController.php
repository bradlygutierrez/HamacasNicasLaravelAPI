...
class ProformaController extends Controller
{
    ...
    {
        $this->assertOwner($request, $proforma); return new ProformaResource($proforma->load(['cliente', 'vendedor', 'pedido', 'detalles.servicios', 'detalles.hamaca.colores', 'servicios', 'materialesSnapshot', 'manoObraSnapshot']));
    }
    ...
    public function products(Request $request)
    {
        ...
        $products = Hamaca::query()->with(['categoria', 'tamano', 'colores'])->whereHas('recetaActiva')->when($request->filled('search'), function ($query) use ($request): void {
            $search = '%' . $request->string('search') . '%';
            $query->where(function ($query) use ($search): void {
                $query->where('nombre', 'like', $search)->orWhereHas('categoria', fn ($x) => $x->where('nombre', 'like', $search))->orWhereHas('tamano', fn ($x) => $x->where('nombre', 'like', $search))->orWhereHas('colores', fn ($color) => $color->where('nombre', 'like', $search));
            });
        })->latest()->paginate($perPage);
        return response()->json(['data' => $products->getCollection()->map(fn (Hamaca $hamaca) => ['id' => $hamaca->id, 'nombre' => $hamaca->nombre, 'colores' => $hamaca->colores->pluck('nombre')->values(), 'categoria' => $hamaca->categoria?->nombre, 'tamano' => $hamaca->tamano?->nombre, 'precio' => $hamaca->precio, 'tiene_receta_activa' => true]), 'meta' => ['current_page' => $products->currentPage(), 'last_page' => $products->lastPage(), 'per_page' => $products->perPage(), 'total' => $products->total()]]);
    }
