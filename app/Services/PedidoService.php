...
class PedidoService
{
    ...
    public function convertFromProforma(Proforma $proforma, Usuario $user, array $data = []): array
    {
        return DB::transaction(function () use ($proforma, $user, $data): array {
            ...
            foreach ($proforma->detalles as $detail) { $line = $pedido->detalles()->create($detail->only(['proforma_detalle_id','hamaca_id','hamaca_variante_id','receta_hamaca_id','receta_version_snapshot','hamaca_nombre_snapshot','hamaca_descripcion_snapshot','cantidad','precio_unitario','descuento','subtotal','costo_unitario_estimado','costo_total_estimado'])); foreach ($detail->servicios as $service) $line->servicios()->create($service->only(['servicio_adicional_id','servicio_nombre_snapshot','alcance_snapshot','metodo_calculo_snapshot','unidad_snapshot','detalle','cantidad','precio_unitario','descuento','subtotal','costo_base_unitario_override','costo_base_unitario_snapshot','costo_unitario_estimado','costo_total_estimado']) + ['proforma_detalle_servicio_id' => $service->id]); }
    ...
    {
        return DB::transaction(function () use ($pedido, $state, $user, $comment): Pedido { $pedido = Pedido::query()->lockForUpdate()->findOrFail($pedido->id); $this->assertStateAccess($pedido, $state, $user); $allowed = ['pendiente' => ['materiales_pendientes', 'materiales_listos', 'cancelado'], 'materiales_pendientes' => ['materiales_listos', 'cancelado'], 'materiales_listos' => ['en_produccion', 'cancelado'], 'en_produccion' => ['terminado', 'cancelado']]; if (!in_array($state, $allowed[$pedido->estado] ?? [], true)) throw new BusinessRuleException('La transición del pedido no es válida.'); if ($state === 'materiales_listos' && !$this->materials->allReady($pedido)) throw new BusinessRuleException('Todos los materiales deben estar listos.'); if ($state === 'en_produccion') $pedido->fecha_inicio_produccion ??= now(); if ($state === 'terminado' && !$this->processes->allComplete($pedido)) throw new BusinessRuleException('Todos los procesos deben estar completados.'); if ($state === 'terminado') $pedido->fecha_terminado = now(); if ($state === 'cancelado') { if ($user->rol !== 'admin') throw new BusinessRuleException('Solo admin puede cancelar pedidos.', [], 403); if (!$comment) throw new BusinessRuleException('El motivo de cancelación es obligatorio.', [], 422); $pedido->cancelado_at = now(); $pedido->cancelado_por_id = $user->id; $pedido->motivo_cancelacion = $comment; } $previous = $pedido->estado; $pedido->estado = $state; $pedido->save(); $this->history($pedido, $previous, $state, $user, $comment); return $pedido->fresh($this->relations()); });
    }

    public function updateLogistics(Pedido $pedido, array $data, Usuario $user): Pedido { if (!in_array($user->rol, ['admin', 'almacenista'], true)) throw new BusinessRuleException('No tenés permiso para actualizar logística.', [], 403); $pedido->update(array_intersect_key($data, array_flip(['fecha_entrega_estimada', 'observaciones_internas']))); return $pedido->fresh($this->relations()); }
