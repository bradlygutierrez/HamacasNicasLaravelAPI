...
class ProformaPricingService
{
    ...
    public function calculatePayload(array $payload, Usuario $user, ?array $authorizedSettings = null): array
    {
        ...
        foreach ($payload['detalles'] ?? [] as $detailIndex => $input) {
            $hamaca = Hamaca::with(['colores', 'recetaActiva.detallesMateriales.material', 'recetaActiva.detallesManoObra.proceso'])->findOrFail($input['hamaca_id']);
            $recipe = $hamaca->recetaActiva;
            if (!$recipe) throw new BusinessRuleException('La hamaca seleccionada no tiene una fórmula activa.', [], 422);
